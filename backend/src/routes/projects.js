...
router.post(
  '/bulk-import',
  wrapAsync(async (req, res) => {
    await ProjectsService.bulkImport(req, res, true, req.headers.referer);
    const payload = true;
    res.status(200).send(payload);
  }),
);
...
router.get(
  '/count',
  wrapAsync(async (req, res) => {
    const payload = await ProjectsDBApi.findAll(req.query, { countOnly: true });
...
router.get(
  '/:id',
  wrapAsync(async (req, res) => {
    const payload = await ProjectsDBApi.findBy({ id: req.params.id });

 ...
 *            type: string
 *            default: trigger_interval_unit
 *          trigger_restricted_dow:
 *            type: string
 *            default: trigger_restricted_dow
 *          trigger_mode:
 *            type: string
 *            default: trigger_mode
 *          decision_path:
 *            type: string
 *            default: decision_path
 *          temp_id:
 *            type: string
 *            default: temp_id
 *          channel:
...
router.post(
  '/bulk-import',
  wrapAsync(async (req, res) => {
    await EventsService.bulkImport(req, res, true, req.headers.referer);
    const payload = true;
    res.status(200).send(payload);
  }),
);
...
router.get(
  '/count',
  wrapAsync(async (req, res) => {
    const payload = await EventsDBApi.findAll(req.query, { countOnly: true });
...
router.get(
  '/:id',
  wrapAsync(async (req, res) => {
    const payload = await EventsDBApi.findBy({ id: req.params.id });

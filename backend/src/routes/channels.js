...
router.post(
  '/bulk-import',
  wrapAsync(async (req, res) => {
    await ChannelsService.bulkImport(req, res, true, req.headers.referer);
    const payload = true;
    res.status(200).send(payload);
  }),
);
 ...
 *        - bearerAuth: []
 *      tags: [Channels]
 *      summary: Count all channels
 *      description: Count all channels
 *      responses:
 *        200:
 *          description: Channels count successfully received
 *          content:
 *            application/json:
 *              schema:
 *                type: array
...
router.get(
  '/count',
  wrapAsync(async (req, res) => {
    const payload = await ChannelsDBApi.findAll(req.query, { countOnly: true });
...
/**
 *  @swagger
 *  /api/channels/autocomplete:
 *    get:
 *      security:
 *        - bearerAuth: []
 *      tags: [Channels]
 *      summary: Find all channels that match search criteria
 *      description: Find all channels that match search criteria
 *      responses:
 *        200:
 *          description: Channels list successfully received
 *          content:

...
router.post(
  '/bulk-import',
  wrapAsync(async (req, res) => {
    await GoalsService.bulkImport(req, res, true, req.headers.referer);
    const payload = true;
    res.status(200).send(payload);
  }),
);
 ...
 *        - bearerAuth: []
 *      tags: [Goals]
 *      summary: Count all goals
 *      description: Count all goals
 *      responses:
 *        200:
 *          description: Goals count successfully received
 *          content:
 *            application/json:
 *              schema:
 *                type: array
...
router.get(
  '/count',
  wrapAsync(async (req, res) => {
    const payload = await GoalsDBApi.findAll(req.query, { countOnly: true });
...
/**
 *  @swagger
 *  /api/goals/autocomplete:
 *    get:
 *      security:
 *        - bearerAuth: []
 *      tags: [Goals]
 *      summary: Find all goals that match search criteria
 *      description: Find all goals that match search criteria
 *      responses:
 *        200:
 *          description: Goals list successfully received
 *          content:

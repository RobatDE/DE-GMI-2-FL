...
module.exports = class UsersDBApi {
  ...
  static async findBy(where, options) {
    ...
    output.assets_created_by_user = await users.getAssets_created_by_user({
      transaction,
    });

    output.assets_checked_out_by_user =
      await users.getAssets_checked_out_by_user({
        transaction,
      });

    output.categories_created_by_user =
      await users.getCategories_created_by_user({
        transaction,
      });

    output.categories_modified_by_user =
      await users.getCategories_modified_by_user({
        transaction,
      });

    output.categories_checked_out_by_user =
      await users.getCategories_checked_out_by_user({
        transaction,
      });

    output.channels_owner = await users.getChannels_owner({
      transaction,
    });

    output.companies_owner = await users.getCompanies_owner({
      transaction,
    });

    output.companies_created_by_user = await users.getCompanies_created_by_user(
      {
        transaction,
      },
    );

    output.companies_modified_by_user =
      await users.getCompanies_modified_by_user({
        transaction,
      });

    output.companies_checked_out_by_user =
      await users.getCompanies_checked_out_by_user({
        transaction,
      });

    output.programs_created_by_user = await users.getPrograms_created_by_user({
      transaction,
    });

    output.programs_modified_by_user = await users.getPrograms_modified_by_user(
      {
        transaction,
      },
    );

    output.campaigns_created_by_user = await users.getCampaigns_created_by_user(
      {
        transaction,
      },
    );

    output.campaigns_modified_by_user =
      await users.getCampaigns_modified_by_user({
        transaction,
      });

    output.campaigns_checked_out_by_user =
      await users.getCampaigns_checked_out_by_user({
        transaction,
      });

    output.projects_created_by_user = await users.getProjects_created_by_user({
      transaction,
    });

    output.projects_modified_by_user = await users.getProjects_modified_by_user(
      {
        transaction,
      },
    );

    output.tasks_owner = await users.getTasks_owner({
      transaction,
    });

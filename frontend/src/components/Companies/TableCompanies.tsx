...
import {
  ...
  deleteItem,
  setRefetch,
} from '../../stores/companies/companiesSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureCompaniesCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';

const perPage = 5;

const TableSampleCompanies = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (companiesNotify.showNotification) {
      notify(
        companiesNotify.typeNotification,
        companiesNotify.textNotification,
      );
    }
  }, [companiesNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/companies/companies-view/?id=${id}`);
  };
...
export default TableSampleCompanies;

...
import {
  ...
  deleteItem,
  setRefetch,
} from '../../stores/programs/programsSlice';
...
const TableSamplePrograms = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (programsNotify.showNotification) {
      notify(programsNotify.typeNotification, programsNotify.textNotification);
    }
  }, [programsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/programs/programs-view/?id=${id}`);
  };
...
export default TableSamplePrograms;

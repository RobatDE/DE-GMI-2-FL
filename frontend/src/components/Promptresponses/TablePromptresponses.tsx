...
import { loadColumns } from './configurePromptresponsesCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';

const perPage = 5;

const TableSamplePromptresponses = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  const {
    ...
    loading,
    count,
    notify: promptresponsesNotify,
    refetch,
  } = useAppSelector((state) => state.promptresponses);
  ...
  useEffect(() => {
    if (promptresponsesNotify.showNotification) {
      notify(
        promptresponsesNotify.typeNotification,
        promptresponsesNotify.textNotification,
      );
    }
  }, [promptresponsesNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/promptresponses/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/promptresponses/promptresponses-view/?id=${id}`);
  };
...
export default TableSamplePromptresponses;

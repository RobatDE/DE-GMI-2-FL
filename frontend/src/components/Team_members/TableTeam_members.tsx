...
const TableSampleTeam_members = ({ filterItems, setFilterItems, filters }) => {
  ...
  const {
    ...
    loading,
    count,
    notify: team_membersNotify,
    refetch,
  } = useAppSelector((state) => state.team_members);
  ...
  useEffect(() => {
    if (team_membersNotify.showNotification) {
      notify(
        team_membersNotify.typeNotification,
        team_membersNotify.textNotification,
      );
    }
  ...
  const handleEditAction = (id: string) => {
    router.push(`/team_members/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/team_members/team_members-view/?id=${id}`);
  };

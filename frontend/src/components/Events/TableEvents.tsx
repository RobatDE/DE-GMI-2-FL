...
import {
  ...
  deleteItem,
  setRefetch,
} from '../../stores/events/eventsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureEventsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';

const perPage = 5;

const TableSampleEvents = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (eventsNotify.showNotification) {
      notify(eventsNotify.typeNotification, eventsNotify.textNotification);
    }
  }, [eventsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/events/events-view/?id=${id}`);
  };
...
export default TableSampleEvents;

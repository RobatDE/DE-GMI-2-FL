...
import { loadColumns } from './configurePersonasCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';

const perPage = 5;

const TablePersonas = ({ filterItems, setFilterItems, filters }) => {
  ...
  const {
    ...
    notify: usersNotify,
    refetch,
  } = useAppSelector((state) => state.personas);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/personas/personas-view/?id=${id}`);
  };
  ...
  useEffect(() => {
    loadColumns(
      ...
      setColumns(newCols); 
      console.log('personas loadcolumns  data: ', newCols)
    });
    
  }, []);

  const handleTableSubmit = async (id: string, data) => {
    console.log('personas table submit id: ', id);
    console.log('personas data: ', JSON.stringify(data));
  ...
  console.log('table personas: ', personas);

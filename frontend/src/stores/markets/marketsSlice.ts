...
export const uploadCsv = createAsyncThunk(
  ...
  async (file: File, { rejectWithValue }) => {
    try {
      ...
      const result = await axios.post('markets/bulk-import', data, {
        headers: {
...
export const marketsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(deleteItem.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, `${'Markets'.slice(0, -1)} has been deleted`);
    });
    ...
    builder.addCase(create.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, `${'Markets'.slice(0, -1)} has been created`);
    });
    ...
    builder.addCase(update.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, `${'Markets'.slice(0, -1)} has been updated`);
    });
    ...
    builder.addCase(uploadCsv.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, 'Markets has been uploaded');
    });
...
export const { setRefetch } = marketsSlice.actions;

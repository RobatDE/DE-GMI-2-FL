...
export const uploadCsv = createAsyncThunk(
  ...
  async (file: File, { rejectWithValue }) => {
    try {
      ...
      const result = await axios.post('campaigns/bulk-import', data, {
        headers: {
...
export const campaignsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(deleteItem.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, `${'Campaigns'.slice(0, -1)} has been deleted`);
    });
    ...
    builder.addCase(create.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, `${'Campaigns'.slice(0, -1)} has been created`);
    });
    ...
    builder.addCase(update.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, `${'Campaigns'.slice(0, -1)} has been updated`);
    });
    ...
    builder.addCase(uploadCsv.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, 'Campaigns has been uploaded');
    });
...
export const { setRefetch } = campaignsSlice.actions;
